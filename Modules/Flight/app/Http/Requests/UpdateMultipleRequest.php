...
class UpdateMultipleRequest extends FormRequest
{
    public function rules(): array
    {
        return [
            'ids' => 'required|array',
            'ids.*' => 'exists:flights,id|integer',
            'load' => 'sometimes|integer|min:1',
            'number' => 'sometimes|string|max:50|unique:flights,number',
    ...
    {
        return [
            'flight_ids.required' => 'لیست پروازها الزامی است.',
            'flight_ids.array'    => 'لیست پروازها باید به صورت آرایه ارسال شود.',

            'flight_ids.*.exists' => 'یکی از پروازهای انتخاب شده در سیستم وجود ندارد.',
            'flight_ids.*.integer'=> 'شناسه هر پرواز باید عدد صحیح باشد.',
